...
ip netns exec $NETNS ip link delete $HWLINK &> /dev/null
# force deletion of the old namespace
ip net del $NETNS &> /dev/null
...
ip link delete $MACVLN &> /dev/null
...
ip route del $VPEER_ADDR dev $VETH &> /dev/null
ip route add $VPEER_ADDR dev $VETH

# Test connectivity from namespace to peer (host)
ip netns exec $NETNS ping -q -c 1 ${VETH_ADDR} &> /dev/null
